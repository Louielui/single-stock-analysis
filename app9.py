...
# --- 市場購買力分析 ---
with st.container():
    st.subheader("📉 市場購買力分析")
    volume_ma5 = hist['Volume'].rolling(5).mean()
    volume_ma20 = hist['Volume'].rolling(20).mean()
    vol_today = hist['Volume'].iloc[-1]
    ma5 = volume_ma5.iloc[-1]
    ma20 = volume_ma20.iloc[-1]
    vol_vs_ma = (vol_today - ma5) / ma5 * 100 if ma5 > 0 else 0

    # OBV 計算
    obv = (hist['Close'].diff() > 0).astype(int) * hist['Volume']
    obv -= (hist['Close'].diff() < 0).astype(int) * hist['Volume']
    obv = obv.cumsum()

    # 資金流向判斷
    obv_trend = "上升" if obv.iloc[-1] > obv.iloc[-5] else "下降"
    obv_msg = "OBV 多頭排列 → 資金呈淨流入" if obv_trend == "上升" else "OBV 空頭排列 → 資金呈淨流出"

    # 文字評語
    if vol_today > ma5 and change_percent > 0:
        comment = "放量上攻，買氣強，策略可偏多"
    elif vol_today < ma5 and change_percent < 0:
        comment = "縮量下跌，空方無力，可觀察支撐"
    else:
        comment = "量價平衡，建議區間觀察"

    st.write(f"成交量：{vol_today:,}（較 MA5 {'增加' if vol_vs_ma > 0 else '減少'} {abs(vol_vs_ma):.1f}%）")
    st.write(obv_msg)
    st.info(f"💡 判斷：{comment}")

    fig_vol, ax_vol = plt.subplots(figsize=(10, 3))
    ax_vol.plot(hist.index, volume_ma5, label='Volume MA5', linestyle='--')
    ax_vol.plot(hist.index, volume_ma20, label='Volume MA20', linestyle=':')
    ax_vol.bar(hist.index, hist['Volume'], label='Volume', alpha=0.4)
    ax_vol.set_title('成交量與均量')
    ax_vol.legend()
    st.pyplot(fig_vol)

    fig_obv, ax_obv = plt.subplots(figsize=(10, 2))
    ax_obv.plot(obv.index, obv, label='OBV', color='darkgreen')
    ax_obv.set_title('OBV 累積能量線')
    ax_obv.legend()
    st.pyplot(fig_obv)
